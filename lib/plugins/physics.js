...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot.elytraFly = async () => {
    ...
    // Prefer bot.registry: on runtime-state editions (Bedrock) require('minecraft-data')(bot.version) resolves null,
    // which then throws on effectsByName. Matches the prismarine-physics fork's PlayerState (physics #152/#154).
