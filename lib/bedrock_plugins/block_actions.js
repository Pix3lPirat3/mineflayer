...
module.exports = function inject (bot) {
  ...
  // block playing. type 'change_state' (1) carries the new state in `data`; type 'sound' (0) is the note block's note.
  // Route by the block at the position to the matching Java event.
  bot._client.on('block_event', packet => {
    const p = packet.position; if (!p) return
    const block = blockAt(p); if (!block) return
    const name = block.name || ''
    if (packet.type === 'change_state' || packet.type === 1) {
      if (/chest|barrel|shulker/.test(name)) bot.emit('chestLidMove', block, packet.data, null) // data = viewers/open state
      else if (/piston/.test(name)) bot.emit('pistonMove', block, packet.data, 0) // data = direction/state
    } else if (packet.type === 'sound' || packet.type === 0) {
      if (/note/.test(name)) bot.emit('noteHeard', block, Math.floor((packet.data || 0) / 12), (packet.data || 0) % 12) // best-effort instrument/pitch
    }
  })
