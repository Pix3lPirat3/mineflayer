...
function inject (bot) {
  ...
  const findIngredient = (ing) => {
    ...
    if (ing.tag) {
      // Prefer a real registry tag -> member-items map when available (accurate); otherwise fall back to the
      // name-substring heuristic (planks/logs/wool-style tags). Bedrock crafting_data usually lists concrete items, so
      // the tag path is rare, but this resolves it correctly when the registry exposes tags.
      const tag = ing.tag.replace(/^minecraft:/, '')
      const tags = bot.registry.itemTags || bot.registry.tags?.item || null
      const members = tags && (tags[ing.tag] || tags[tag])
      if (Array.isArray(members) && members.length) {
        const names = new Set(members.map(n => String(n).replace(/^minecraft:/, '')))
        const m = items.find(i => i.name && names.has(i.name))
        if (m) return m
      }
      const base = tag.replace(/s$/, '')
      return items.find(i => i.name && i.name.includes(base))
    }
    return null
  }
