...
function inject (bot) {
  ...
  async function decodeLevelChunk (packet) {
    ...
    // Bedrock 1.18+ can defer subchunks: a NEGATIVE sub_chunk_count is the subchunk-request mode (-1 LIMITED, where
    // highest_subchunk_count is the count; -2 LIMITLESS, where the client requests the dimension's whole vertical range).
    // Older/1.26 fresh worlds send sub_chunk_count >= 0 (full payload) or the 0-with-highest form. Treating -2 as a literal
    // count (the previous `=== 0`-only check) meant networkDecodeNoCache(payload, -2) decoded nothing and no subchunks were
    // ever requested, so 1.19.80-1.21.x worlds came through as all-air.
    const requestMode = packet.sub_chunk_count < 0
    const polling = requestMode || (packet.highest_subchunk_count != null && packet.sub_chunk_count === 0)
    ...
    if (packet.sub_chunk_count === -2) {
      // LIMITLESS: highest_subchunk_count is not the count here; request the full dimension height in sections.
      const height = (bot.game && bot.game.height) || 384
      sectionCount = Math.ceil(height / 16)
    } else {
      sectionCount = packet.highest_subchunk_count // LIMITED (-1) or the 0-with-highest legacy form
    }
