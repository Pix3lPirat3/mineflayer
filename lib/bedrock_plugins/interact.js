...
function inject (bot) {
  ...
  bot.activateItem = (offhand = false) => {
    bot._usingItem = true // feeds the Java-parity bot.usingHeldItem getter; cleared by deactivateItem/completed_using_item
    const held = heldItem()
    sendItemUseAir(held)
    bot.swingArm('right', false)
    // Firework boost while gliding: set bot.fireworkRocketDuration from the rocket's Flight (Java parity). The
    // client_input physics loop counts it down each tick and clears it on landing.
    if (held && /firework/.test(held.name || '') && bot.entity && bot.entity.elytraFlying) {
      bot.fireworkRocketDuration = 10 * (fireworkFlight(held) + 1)
    }
  }
  // Best-effort read of a firework rocket's Flight duration from its NBT (Bedrock stores it under Fireworks.Flight);
  // defaults to 1 when the NBT is not present on the item, which matches a plain crafted rocket.
  function fireworkFlight (item) {
    try {
      const nbt = item && item.nbt && item.nbt.value ? item.nbt.value : null
      const fw = nbt && (nbt.Fireworks || nbt.fireworks)
      const flight = fw && fw.value && (fw.value.Flight || fw.value.flight)
      const v = flight && (flight.value ?? flight)
      const n = Number(v)
  ...
  function stopUsing () { if (usingItemTimer) { clearInterval(usingItemTimer); usingItemTimer = null }; bot._usingItem = false }
  ...
  // Mount a rideable entity (horse, boat, minecart, ...). On Bedrock that is just interacting with it (same path as
  // useOn); the server establishes the ride with set_entity_link, which the entities plugin turns into bot.vehicle +
  // the 'mount' event. Await 'mount' if you need to know it took.
  bot.mount = (entity) => {
    const target = typeof entity === 'object' ? entity : bot.entities[entity]
    if (!target) throw new Error('mount: unknown entity')
    bot.useOn(target)
  }

  // Leave the current vehicle. Bedrock: an interact packet with action_id 'leave_vehicle' targeting the vehicle; the
  // server replies with a set_entity_link remove, which clears bot.vehicle and fires 'dismount'.
  bot.dismount = () => {
    const vehicle = bot.vehicle
    if (!vehicle) return
    send('interact', { action_id: 'leave_vehicle', target_entity_id: toBig(vehicle.id ?? 0), has_position: false })
  }
  ...
  bot._genericPlace = async (referenceBlock, faceVector, options = {}) => {
    ...
    bot.swingArm(options.swingArm || 'right', false)
