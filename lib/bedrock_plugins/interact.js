...
function inject (bot) {
  ...
  bot.placeBlock = async (referenceBlock, faceVector) => {
    ...
    const empty = { network_id: 0, count: 0, metadata: 0, has_stack_id: false, block_runtime_id: 0 }
    const newCount = Math.max(0, (item?.count ?? 1) - 1)
    const oldItem = { ...held }
    const newItem = newCount <= 0 ? empty : { ...held, count: newCount }
    send('inventory_transaction', {
      transaction: {
        legacy: { legacy_request_id: 0 },
        transaction_type: 'item_use',
        actions: [{ source_type: 'container', window_id: 0, slot, old_item: oldItem, new_item: newItem }],
        transaction_data: {
