...
function inject (bot) {
  ...
  bot._client.on('take_item_entity', packet => {
    ...
    if (collector && collected) {
      // When the bot itself is the collector, add the item to its inventory: Bedrock sends no inventory update for a
      // ground pickup, only this packet, and the item type/count came with the earlier add_item_entity.
      if (bot.entity && collector.id === bot.entity.id && collected.item && typeof bot._addInventoryItem === 'function') {
        bot._addInventoryItem(collected.item)
      }
