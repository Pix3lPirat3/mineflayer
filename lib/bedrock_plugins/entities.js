...
function inject (bot) {
  ...
  bot._client.on('set_entity_link', packet => {
    ...
    if (passenger === bot.entity) {
      if (passenger.vehicle) { bot.vehicle = vehicle; bot.emit('mount') } else { const was = bot.vehicle; bot.vehicle = null; bot.emit('dismount', was) }
    }
  })
  ...
  function applyMetadata (entity, metadata = []) {
    ...
    // Bedrock keys metadata by string names ('nametag', 'flags', ...); prismarine-entity's getCustomName() and Java
    // ecosystem code read the NUMERIC index 2 for the custom name. Bridge the Bedrock 'nametag' onto metadata[2] so
    // entity.getCustomName() returns the name (a ChatMessage) instead of always null. An empty tag clears it.
