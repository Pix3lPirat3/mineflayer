...
function inject (bot) {
  ...
  bot._client.on('update_trade', packet => {
    ...
    bot.trades = recipes.map(r => {
      ...
      return {
        ...
        maximumNbTradeUses: maxUses, // Java field name (mineflayer-java Trade.maximumNbTradeUses); keep both spellings
        nbTradeUses: uses,
    ...
    // Java's openVillager resolves to a real Window (EventEmitter with .close() and a .selectedTrade); make tradeWindow an
    // EventEmitter so `villager.once('close', ...)` / `.close()` do not throw on a bedrock bot.
    bot.tradeWindow = Object.assign(new EventEmitter(), {
      id: packet.window_id,
      ...
      // Close the underlying trading UI (mineflayer-java's window.close()).
      close: () => { if (typeof bot.closeWindow === 'function' && bot.currentWindow) return bot.closeWindow(bot.currentWindow) }
    })
  ...
  bot.trade = async (a, b, c) => {
    ...
    bot.tradeWindow.selectedTrade = trade // Java parity: window.selectedTrade is the trade being executed
