...
function inject (bot) {
  ...
  // + cause). The server responds with a rejection (status 2) unless the beacon is a fully POWERED beacon the server has
  // registered as active - in local testing a command-built pyramid (/fill or per-block /setblock, beacon-first or base-first,
  // creative or survival, sky cleared, powered-up wait) was not accepted, so this needs a genuinely player-built/recognized
  // beacon to reach status ok. The helper itself is capture-verified. primary/secondary are Bedrock effect ids (1 speed, 3 haste).
