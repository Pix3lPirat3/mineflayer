...
function inject (bot) {
  ...
  // Beacon: place an ingot -> beacon_payment[27], then one item_stack_request carrying the chosen effects. The request
  // structure now matches the real-client capture exactly (beacon_payment{primary,secondary:0 when none} > destroy, with
  // the placed stack_id), and the server responds (no longer a silent drop). Full activation still needs a properly
  // POWERED beacon (a complete pyramid with sky access that the server has registered) - a /fill'd pyramid can return a
  // generic reject until the beacon scans its base. primary/secondary are Bedrock effect ids (e.g. 1 speed, 3 haste).
