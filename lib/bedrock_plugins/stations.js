...
function inject (bot) {
  ...
  bot.enchant = async (tableBlock, item, choice = 0) => {
    ...
    const resultDescriptor = {
      type: 'name', legacy_type: 1, name, metadata: src.metadata ?? 0, count: 1, block_runtime_id: 0,
  ...
  bot.loom = async (block, banner, dye, pattern = 'cre') => {
    ...
    const resultDescriptor = {
      type: 'name', legacy_type: 1, name: bannerName, metadata: b.metadata ?? 0, count: 1, block_runtime_id: 0,
