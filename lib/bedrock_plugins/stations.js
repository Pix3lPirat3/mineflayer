...
function inject (bot) {
  ...
  // Beacon: place an ingot -> beacon_payment[27], then one item_stack_request carrying the chosen effects. The request is
  // byte-exact to the real-client capture (beacon_payment{primary,secondary:0 when none} > destroy with the placed stack_id,
  // + cause). It returns a consistent status-2 rejection that is NOT explained by the beacon build: verified live (local BDS
  // 1.26.45) with the WHOLE beacon placed by the bot through the real placement path (9/9 iron blocks + the beacon,
  // physically) and every other variable tried (/fill and /setblock bases, creative and survival, effects 1 and 3, sky
  // cleared, up to 6s power-up) - all status 2. Since the request is byte-exact to a real working client, the remaining
  // difference is likely a protocol/serialization detail or a BDS beacon quirk, and needs a WORKING-beacon relay capture to
  // diff against (the method that cracked loom). primary/secondary are Bedrock effect ids (1 speed, 3 haste).
