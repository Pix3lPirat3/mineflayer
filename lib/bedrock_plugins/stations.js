...
function inject (bot) {
  ...
  // Java-parity computed layer: mineflayer-java's enchant window exposes enchantments[0..2] {level, expected:{enchant,
  // level}} plus an 'xpseed' and a 'ready' event. Build the same shape from Bedrock's player_enchant_options
  // (EnchantOption{cost = the level requirement, self_enchants = the applied set, name, option_id}) and attach it to the
  // open enchant window. xpseed has no Bedrock equivalent (the client is never told the seed), so it stays null.
  const buildEnchantLayer = (options) => (options || []).map((o) => {
    const first = (o.self_enchants && o.self_enchants[0]) || (o.held_enchants && o.held_enchants[0]) || null
    return {
      level: o.cost,
      expected: first ? { enchant: first.id, level: first.level } : null,
      enchants: (o.self_enchants || []).map(e => ({ id: e.id, level: e.level })),
      name: o.name,
      optionId: o.option_id
    }
  })
  bot._client.on('player_enchant_options', (packet) => {
    bot.enchantmentOptions = packet.options || []
    const w = bot.currentWindow
    if (w && /enchant/.test(w.type || '')) {
      w.enchantments = buildEnchantLayer(bot.enchantmentOptions)
      w.xpseed = null
      if (typeof w.emit === 'function') w.emit('ready')
    }
    bot.emit('enchantmentOptionsReady', bot.enchantmentOptions)
  })
  ...
  bot.enchant = async (tableBlock, item, choice = 0) => {
    ...
    // The explicit lapis consume is version-dependent (both verified live with a real-client relay capture on 1.26.45):
    //  - 1.26.51+ (protocol >= 2193) REQUIRES it, or the craft is rejected with status 18 (lapis slot not fully consumed).
    //  - 1.26.45 (protocol 2169) consumes the lapis itself and REJECTS an explicit consume with status 24 (a slot-state
    //    mismatch). A real 1.26.45 client sends no lapis consume at all.
    // Placing EXACTLY (choice+1) lapis means the slot ends fully consumed either way, so only the action differs.
    const protocol = (bot.registry.version && bot.registry.version.version) || 0
    const sendLapisConsume = protocol >= 2193
    const craft = await stackReq((rid) => [
      ...
      ...((lapisStackId && sendLapisConsume) ? [{ type_id: 'consume', legacy_type_id: 5, count: lapisNeeded, source: { slot_type: { container_id: 'enchanting_lapis' }, slot: 15, stack_id: lapisStackId } }] : []),
