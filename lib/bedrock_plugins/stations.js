...
function inject (bot) {
  ...
  // NOTE (2026-09-21): structure matches the relay capture, but tower BDS 1.26.51 silently drops the commit (no
  // item_stack_response) even for a basic pattern - a craft_loom_request detail still to root-cause. Ported but NOT yet
  // live-verified (unlike enchant/anvil/grindstone). Left in place for follow-up; niche (not a mineflayer-java method).
  bot.loom = async (block, banner, dye, pattern = 'cre') => {
    ...
    const { response, status } = await stackReq((rid) => [
      { type_id: 'craft_loom_request', legacy_type_id: 15, pattern, times_crafted: 1 },
      { type_id: 'results_deprecated', legacy_type_id: 19, times_crafted: 1, result_items: [resultDescriptor] },
      { type_id: 'consume', legacy_type_id: 5, count: 1, source: { slot_type: { container_id: 'loom_input' }, slot: 9, stack_id: b.stackId ?? 0 } },
      { type_id: 'consume', legacy_type_id: 5, count: 1, source: { slot_type: { container_id: 'loom_dye' }, slot: 10, stack_id: d.stackId ?? 0 } },
      { type_id: 'take', legacy_type_id: 0, count: 1, source: { slot_type: { container_id: 'creative_output' }, slot: 50, stack_id: rid }, destination: { slot_type: { container_id: invContainer(dest) }, slot: dest, stack_id: 0 } }
    ])
  ...
  // caller takes the bottles when done. Capture-derived (STATIONS.md, 09-28-57); NOT yet live-verified (needs a real
  // brewing setup) - reachable today via bot.openContainer + raw moves, this is the thin helper.
  ...
  // Beacon: place an ingot -> beacon_payment[27], then one item_stack_request carrying the chosen effects. Capture-derived
  // (STATIONS.md); NOT yet live-verified (needs a built beacon pyramid). primary/secondary are Bedrock effect ids.
  bot.beaconActivate = async (block, primaryEffect, secondaryEffect = -1, payment) => {
    ...
    if (pay) {
      ...
      if (p.status === 'ok') { if (pay.count > 1) { pay.count -= 1; bot.inventory.updateSlot(pay.slot, pay) } else bot.inventory.updateSlot(pay.slot, null) }
      await wait(120)
    }
    const { status } = await stackReq([
      { type_id: 'beacon_payment', legacy_type_id: 21, primary_effect: primaryEffect, secondary_effect: secondaryEffect },
      { type_id: 'destroy', legacy_type_id: 4, count: 1, source: { slot_type: { container_id: 'beacon_payment' }, slot: 27, stack_id: pay ? (pay.stackId ?? 0) : 0 } }
    ])
    if (status !== 'ok') throw new Error(`beaconActivate: rejected (status ${status})`)
    return true
  }
  // Smithing (netherite upgrade / trim): place base -> smithing_table_input[51], material -> smithing_table_material[52]
  // (+ template -> smithing_table_template), then a craft_recipe whose recipe_network_id is the smithing recipe's id. That
  // recipe id is NOT in the derived notes yet (the 09-28-57 capture recorded the slots, not a completed survival craft), so
  // this is intentionally not wired as a one-call helper - do it via bot.openContainer + raw moves + the recipe id from a
  // crafting_data capture until that id is confirmed live. Tracked in STATIONS.md.
}
