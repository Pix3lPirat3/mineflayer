...
function inject (bot) {
  ...
  // Creative flight (mineflayer-java bot.creative.startFlying/stopFlying/flyTo). On Bedrock the client toggles flight
  // with request_ability(flying); the server accepts it when may_fly is granted (creative/spectator). flyTo then moves
  // the bot to a point: it suspends the self-simulation (so gravity does not fight the path), steps the position toward
  // the target each tick, and the player_auth_input send loop reports it. Restores flight/physics state when done.
  const { Vec3 } = require('vec3')
  // Enter creative flight: BDS never grants may_fly in the ability flags (verified), so it will not simulate flight for
  // us; instead we suspend the self-simulation (bot._creativeFlying makes client_input skip gravity/collision and ignore
  // the ground correction) and hold/drive the position ourselves. The request_ability keeps the server's view in sync.
  bot.creative.startFlying = () => {
    send('request_ability', { ability: 'flying', value_type: 'bool', bool_value: true, float_val: 0 })
    bot._creativeFlying = true
    if (bot.entity) { bot.entity.flying = true; if (bot.entity.velocity) bot.entity.velocity.set(0, 0, 0) }
  }
  // Leave creative flight: physics resumes, so the bot falls/lands as expected.
  bot.creative.stopFlying = () => {
    send('request_ability', { ability: 'flying', value_type: 'bool', bool_value: false, float_val: 0 })
    bot._creativeFlying = false
    if (bot.entity) bot.entity.flying = false
  }
  // Fly to a point and hover there (stays flying until stopFlying). Steps the position toward the target each tick; the
  // send loop reports it and the ground correction is ignored while flying.
  bot.creative.flyTo = async (destination) => {
    const dest = destination.clone ? destination.clone() : new Vec3(destination.x, destination.y, destination.z)
    if (!bot._creativeFlying) bot.creative.startFlying()
    let guard = 0
    while (bot.entity && bot.entity.position.distanceTo(dest) > 0.6 && guard++ < 400) {
      const pos = bot.entity.position
      const dx = dest.x - pos.x; const dy = dest.y - pos.y; const dz = dest.z - pos.z
      const dist = Math.sqrt(dx * dx + dy * dy + dz * dz) || 1
      const step = Math.min(0.5, dist) // ~10 blocks/s at 20 Hz stepping
      pos.x += (dx / dist) * step
      pos.y += (dy / dist) * step
      pos.z += (dz / dist) * step
