...
function inject (bot) {
  bot.placeEntity = async (referenceBlock, faceVector, options = {}) => {
    ...
    // Java asserts the held item is an entity item (boat/minecart/spawn egg/armor stand/end crystal); without this a bad
    // item just times out with a misleading error, or a nearby unrelated spawn is mis-attributed. Skip the check only if
    // nothing is held (let the server reject) or the caller opts out.
    const held = bot.heldItem
    if (held && options.validateItem !== false && !/boat|minecart|spawn_egg|armor_stand|end_crystal/.test(held.name || '')) {
      throw new Error('placeEntity: held item is not a placeable entity item (' + held.name + ')')
    }
    ...
    bot.emit('entityPlaced', entity) // Java parity: an entity was placed from an item
    return entity
  }
}
