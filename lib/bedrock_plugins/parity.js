...
function inject (bot) {
  ...
  bot.openEnchantmentTable = async (block) => {
    ...
    if (w && !w.targetItem) {
      ...
      // Java-parity granular window methods: place the target/lapis, then read w.enchantments (populated from the server's
      // player_enchant_options), then either commit via w.enchant(choice) or pull the item back with takeTargetItem. These
      // place/take into the enchant slots directly (enchanting_input 14, enchanting_lapis 15).
      const placeInto = async (item, containerId, slot, count) => {
        if (!bot._stack) throw new Error('enchant window: stack_request plugin not loaded')
        const { stackReq, stackIdFor, bedrockOf, invContainer } = bot._stack
        const src = (typeof item === 'object' && item && item.name) ? item : bot.inventory.items().find(i => i.name === item || i.type === item)
        if (!src) throw new Error('enchant window: item not found in inventory')
        const bed = bedrockOf(src.slot)
        const n = count ?? 1
        const { response, status } = await stackReq([{ type_id: 'place', legacy_type_id: 1, count: n, source: { slot_type: { container_id: invContainer(bed) }, slot: bed, stack_id: src.stackId ?? 0 }, destination: { slot_type: { container_id: containerId }, slot, stack_id: 0 } }])
        if (status !== 'ok') throw new Error(`enchant window: place rejected (status ${status})`)
        if (src.count > n) { src.count -= n; bot.inventory.updateSlot(src.slot, src) } else bot.inventory.updateSlot(src.slot, null)
        return stackIdFor(response, containerId, slot) // the authoritative stack id the server gave the placed item
      }
      w.putTargetItem = async (item) => {
        const src = (typeof item === 'object' && item && item.name) ? item : bot.inventory.items().find(i => i.name === item || i.type === item)
        w._targetItem = src ? { type: src.type, metadata: src.metadata ?? 0 } : null
        w._targetStackId = await placeInto(item, 'enchanting_input', 14, 1)
        return true
      }
      w.putLapis = async (item = 'lapis_lazuli', count = 1) => { w._lapisStackId = await placeInto(item, 'enchanting_lapis', 15, count); return true }
      w.takeTargetItem = async () => {
        if (!bot._stack) throw new Error('enchant window: stack_request plugin not loaded')
        const { stackReq, invContainer, firstFreeBedrockSlot, playerSlot } = bot._stack
        const dest = firstFreeBedrockSlot()
        if (dest == null) throw new Error('enchant window: no free inventory slot for the target item')
        const { status } = await stackReq([{ type_id: 'take', legacy_type_id: 0, count: 1, source: { slot_type: { container_id: 'enchanting_input' }, slot: 14, stack_id: w._targetStackId ?? 0 }, destination: { slot_type: { container_id: invContainer(dest) }, slot: dest, stack_id: 0 } }])
        if (status === 'ok') {
          w._targetStackId = undefined
          if (w._targetItem) { const back = new (require('prismarine-item')(bot.registry))(w._targetItem.type, 1, w._targetItem.metadata); bot.inventory.updateSlot(playerSlot(dest), back); w._targetItem = null }
        }
        return status === 'ok'
      }
    }
  ...
  bot.clickWindow = async (slot, mouseButton = 0, mode = 0) => {
    ...
    // mode 2 = number-key / hotbar swap: swap the item at `slot` with hotbar slot `mouseButton` (0-8). Java's window uses
    // this for the "press 1-9 to swap" action. Player-inventory only on bedrock (moveInventoryItem swaps when both slots
    // hold items, or moves when the target is empty); the Java hotbar key is already a bedrock 0-8 hotbar slot.
    if (mode === 2) {
      if (typeof bot.moveInventoryItem !== 'function') return
      const bedrockSlot = (slot >= QUICK_BAR_START) ? slot - QUICK_BAR_START : slot
      bot.moveInventoryItem(bedrockSlot, mouseButton & 7); await wait(200)
      return
    }
    // mode 4 = drop key (Q): mouseButton 0 drops one, 1 drops the whole stack. Bedrock uses toss/tossStack.
    if (mode === 4) {
      const it = ((bot.currentWindow && bot.currentWindow.slots) || bot.inventory.slots)[slot]
      if (!it) return
      if (mouseButton === 1 && typeof bot.tossStack === 'function') { try { await bot.tossStack(it) } catch {} } else if (typeof bot.toss === 'function') { try { await bot.toss(it.type, it.metadata ?? null, 1) } catch {} }
      return
    }
  ...
  // Set a command block's command (Bedrock command_block_update). Position is a Vec3/block position. Java-parity guards:
  // must be in creative, and the target must actually be a command block; trackOutput defaults to false like Java (the
  // bedrock version previously defaulted it true and skipped both guards).
  bot.setCommandBlock = (pos, command, options = {}) => {
    if (bot.game && bot.game.gameMode !== 'creative') throw new Error('setCommandBlock: the bot must be in creative mode')
    const bp = { x: Math.floor(pos.x), y: Math.floor(pos.y), z: Math.floor(pos.z) }
    if (typeof bot.blockAt === 'function') {
      const block = bot.blockAt(new Vec3(bp.x, bp.y, bp.z))
      if (block && block.name && !/command_block/.test(block.name)) throw new Error('setCommandBlock: target block is not a command block (' + block.name + ')')
    }
    const send = (n, p) => { if (typeof bot._client.queue === 'function') bot._client.queue(n, p); else bot._client.write(n, p) }
    // The schema field names/types are exact: mode is a mapped string (impulse/repeat/chain), needs_redstone/conditional
    // are bools. The old code sent mode as an int + redstone_mode/is_conditional, which failed to serialize entirely.
    const MODES = ['impulse', 'repeat', 'chain']
    send('command_block_update', {
      is_block: true,
      position: bp,
      mode: typeof options.mode === 'string' ? options.mode : (MODES[options.mode ?? 0] || 'impulse'),
      needs_redstone: !options.alwaysActive, // Java alwaysActive => does not need redstone
      conditional: !!options.conditional,
      command: command || '',
      last_output: '',
      name: options.name || '',
      filtered_name: '', // 1.26.x adds this string field; omitting it fails serialization
      should_track_output: options.trackOutput ?? false,
