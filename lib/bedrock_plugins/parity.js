...
function inject (bot) {
  ...
  bot.openAnvil = openStationBlock
  bot.openEnchantmentTable = openStationBlock
  ...
  // Java-parity bot.transfer(options): move items between window slot ranges (index.d.ts TransferOptions). Moves whole
  // matching stacks from [sourceStart,sourceEnd) into free/stackable slots in [destStart,destEnd), up to `count` items,
  // over the tested bot.moveSlotItem primitive (which handles the Bedrock item_stack_request translation). Partial-stack
  // splitting is not yet supported (Bedrock can split with a count, but the dest bookkeeping is a later refinement).
  ...
  // Elytra flight is a survival input mode on Bedrock (start_gliding input flag). Not yet wired to the input stream;
  // expose the API with a clear error so callers know it is unported rather than silently no-op.
  bot.elytraFly = async () => { throw new Error('elytraFly is not yet implemented on Bedrock (needs the start_gliding auth-input flag)') }
  ...
  bot.teams = bot.teams || {}
  bot.tablist = bot.tablist || { header: '', footer: '' }
}
