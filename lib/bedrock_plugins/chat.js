...
function inject (bot, options) {
  ...
  // Bedrock knows a message's type (chat/whisper) natively from the 'text' packet, so it emits 'chat'/'whisper' directly.
  // If the caller opts into defaultChatPatterns, chat_patterns ALSO derives those from messagestr - suppress the native
  // emit then so 'chat'/'whisper' fire exactly once (the pattern layer becomes the single source, matching Java).
  const nativeChatEmit = options.defaultChatPatterns !== true
  ...
  bot._client.on('text', packet => {
    ...
    if (packet.type === 'whisper' || packet.type === 'json_whisper') {
      if (nativeChatEmit) bot.emit('whisper', source, message.toString(), message.translate, message, null)
    } else if (position === 'chat') {
      if (nativeChatEmit) bot.emit('chat', source, message.toString(), message.translate, message, null)
    }
  })
