...
function inject (bot) {
  ...
  // Bedrock's book_edit addresses a HOTBAR slot (0-8). Resolve + validate: a main-inventory slot (Java 9-35) has no
  // valid hotbar index, so throw a clear error instead of silently sending an out-of-range inventory_slot (the old
  // hotbarIndex returned the raw 9-35 for those). Callers must hold the book in the hotbar (Java slots 36-44).
  const resolveHotbar = (slot) => {
    const b = slot == null ? bot.quickBarSlot : hotbarIndex(slot)
    if (!(b >= 0 && b <= 8)) throw new Error('book: the book must be in the hotbar (Java slots 36-44); slot ' + slot + ' is main inventory')
    return b
  }
  ...
  bot.writeBook = async (slot, pages) => {
    ...
    const bedrockSlot = resolveHotbar(slot)
