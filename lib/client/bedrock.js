...
const { classicSkin } = require('../skin')
...
function createClient (options) {
  ...
  // Offline logins default to a classic skin: minecraft-data's default is a PERSONA skin that only validates against
  // Xbox auth, so PocketMine/PowerNukkitX reject it (invalidSkin) while BDS accepts it. A classic skin works on every
  // server. Opt out with offlineSkin: 'persona', or supply your own via skinData.
  if (clientOptions.offline && clientOptions.skinData === undefined && clientOptions.offlineSkin !== 'persona') {
    clientOptions.skinData = classicSkin()
  }
  delete clientOptions.offlineSkin // mineflayer-only knob; not a bedrock-protocol option
